            ...
            for img1 in s_1_map:
                ...
                for i,(m,n) in enumerate(matches):
                    if m.distance < ratio_lowe*n.distance:
                        goodmatches += 1
                        score_match.append(math.exp(-a*m.distance))
                    else :
                        score_match.append(0)
                score_total = sum(score_match) / len(score_match)
                proba.append(score_total)

            #tri selon le score moyen le plus grand
            if (sum(proba) != 0):
